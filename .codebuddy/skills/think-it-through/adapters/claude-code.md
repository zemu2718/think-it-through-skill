...
当前可靠加载方式是显式 `/think-it-through`。已加载后的上下文检查点由 v0.4.1 正式合同定义，但不证明自然语言自动加载或真实多轮行为已经通过；冻结自动发现 holdout 仍为正例 1/8、负例 8/8，不能把自然语言自动加载描述为已经通过。
...
调用后记录实际来源、日期、范围和状态。项目可行性搜索、候选核验与最强替代试用只有当前会话真实 tool trace、consent 和 receipt 才算发生。网络搜索不继承安装/执行、账号认证、模型调用或私有数据访问；只读搜索不继承写入、发送或其他外部行动。失败后按证据策略降级并回 Gate-routing；只有另一类尚未处理且有独特增量的 Gate 才继续，否则进入 B。
...
5. 按任务裁剪上下文，并禁止额外 Agent 递归委派；项目可行性反方不接收主判断、偏好方案或完整 transcript；
6. 主 Agent 统一综合并给实际数量回执。

复杂 Workflow 或大规模编排需要用户明确选择，不能由一般 Agent consent、历史偏好或“在上限内自动”推定。保存偏好只影响是否主动询问或默认协作方式；新 Agent 任务仍需当前、具体且范围匹配的授权。项目或用户设置的更严格 Agent 总额度必须同时遵守。

Claude Code 子 Agent 的隔离上下文、工具约束和并行能力不证明结论质量必然提高。不得以 Agent 数或一致意见替代来源和现实证据。

## 真人参与与外部行动

默认只生成可转发材料，不携带执行 consent，也不冒充真人已参与。把具体问题交给具体真人需要 `participation_delegation`；向飞书、邮件、聊天、工单、社交平台或其他外部服务实际发送、邀请或联系，还需要独立、具体、当次的 `external_action`。两项可以同屏解释，但形成两份 consent，实际 operation receipt 同时引用；一个服务或目标的授权不能复用于另一个。
...
工具调用被拒绝后不原样重试；能力不可用时不声称已完成；部分 Agent 失败时保留有效材料并回 Gate-routing，只有另一类尚未处理且有独特增量的 Gate 才继续，否则进入 B。
