...
当前可靠加载方式是显式 `/think-it-through`。已加载后的上下文检查点由 v0.4.0 正式合同定义，但不证明自然语言自动加载或真实多轮行为已经通过；冻结自动发现 holdout 仍为正例 1/8、负例 8/8，不能把自然语言自动加载描述为已经通过。
...
调用后记录实际来源、日期、范围和状态。项目可行性搜索、候选核验与最强替代试用只有当前会话真实 tool trace、consent 和 receipt 才算发生。网络搜索不继承安装/执行、账号认证、模型调用或私有数据访问；只读搜索不继承写入、发送或其他外部行动。失败后按证据策略降级进入 B。
